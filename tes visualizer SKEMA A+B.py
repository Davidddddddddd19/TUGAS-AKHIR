...
def extract_deflection(defl_profile, max_defl, L_mm,
    ...
    """Extract deflection using the specified JSON keys. Sign is kept as-is."""
...
def draw_diagram(ax, x_mm, values, ylabel, color_key):
    ...
    ax.plot(x_mm, values, color=line_col, linewidth=1.8, zorder=3)
    ax.fill_between(x_mm, 0, values, color=fill_col, alpha=0.55, zorder=2)
    ...
    # SAP2000 tension-side convention: positive moment plotted downward
    if color_key == "BMD":
        ax.invert_yaxis()
...
class DiagramPanel(QFrame):
    ...
    def update(self, title, x_mm, values, ylabel):
        self.lbl_title.setText(title)
        self.figure.clear()
        ax = self.figure.add_subplot(1, 1, 1)
        v_max = draw_diagram(ax, x_mm, values, ylabel, self.color_key)
...
class VisualizerWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.btn_next = QPushButton("🔄  Select Next Element")
        self.btn_next.setFixedHeight(34)
        self.btn_next.setStyleSheet(
            "background:#1565C0; color:white; font-weight:bold; font-size:9pt;"
        ...
        btn_png.setStyleSheet(
            "font-size:9pt; border-radius:4px; padding:0 12px;"
    ...
    def _on_export_png(self):
        ...
        xd, d = extract_deflection(dp, md, L_mm, defl_key, defl_prefix)
