    ...
    try:
        ...
        if status == 0:
            ...
            for item in processed_elements:
                ...
                try:
                    if not subs:
                          # RAW EXTRACTION (Verified: ops.eleForce returns GLOBAL Forces for ElasticTimoshenkoBeam)
                          # [F_Global_X, F_Global_Y, F_Global_Z, M_Global_X, M_Global_Y, M_Global_Z]
                          f = ops.eleForce(eid)
                          
                          # Calculate Scientific Local Forces
                          local_axes = item['raw'].get('local_axes', {})
                          local_F = calculate_local_forces(f, local_axes)
                          ...
                          # Handle Sub-Elements - Take FIRST sub-element (Start Node)
                          first_eid = subs[0][0]
